return function()
  ...
  -- File finding
  map('n', '<leader>ff', function() fzf.files({ cmd = "rg --no-ignore-vcs --files" }) end, opts)
  map('n', '<leader>fF', function() 
    fzf.files({ cmd = "rg --no-ignore-vcs --files", search = vim.fn.expand('<cword>') }) 
  end, opts)
  map('n', '<leader>fg', function() 
    fzf.live_grep({ 
      rg_opts = "--no-ignore-vcs --no-heading --with-filename --line-number --column --smart-case" 
    }) 
  end, opts)
  map('n', '<leader>fG', function() 
    fzf.live_grep({ 
  ...
  map('n', '<leader>fS', function() fzf.grep_cword({ fzf_opts = { ["--exact"] = "" } }) end, opts)
